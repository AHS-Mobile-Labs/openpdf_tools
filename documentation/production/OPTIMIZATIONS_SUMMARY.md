...
- **`lib/utils/android_path_helper.dart`** - Android path handling
  - Normalize paths for Android
  - Sanitize filenames for file system
  - Safe file operations on Android
  - Scoped storage support

- **`lib/utils/scoped_storage_helper.dart`** - Android 11+ scoped storage
  - Scoped storage compatibility layer
  - Safe directory access for Android 11+
...
- **`lib/widgets/web_pdf_viewer.dart`** - Cross-platform PDF viewer
  - Platform-specific PDF viewing
  - Web PDF.js integration
  - Mobile/desktop fallbacks
...
- **`lib/models/signing_models.dart`** - Digital signature domain models
  - Type-safe certificate, signing request, and result models
  - Comprehensive validation
  - Immutable data classes with copyWith pattern

- **`lib/services/certificate_service.dart`** - Certificate management
  - Certificate file validation (.p12, .pfx, .pem)
  - Password verification (never logged)
  - Expiry detection and warnings
  - Secure memory clearing

- **`lib/services/secure_file_picker_service.dart`** - Cross-platform file selection
  - PDF file picking with validation
  - Certificate file picking with validation
  - Image file picking for signatures
  - Magic bytes verification
  - File size limit enforcement
  - Safe output directory selection

- **`lib/services/production_pdf_signing_service.dart`** - PDF signing operations
...
- **`lib/screens/sign_pdf_screen_refactored.dart`** - Enterprise signing UI
...
### Tests
- **`test/signing_models_test.dart`** - Comprehensive model tests
  - 40+ unit tests for signing models
...
## 6. PDF Digital Signing Feature (NEW)

### Enterprise-Grade Security
- ✅ Certificate validation (.p12, .pfx, .pem formats)
- ✅ Password security (never logged, memory cleared)
- ✅ SHA-256 signature generation
- ✅ File size limits enforced (5MB certs, 500MB PDFs)
- ✅ Magic bytes verification for all files
- ✅ Corruption detection
- ✅ Certificate expiry detection (30-day warnings)

### Multi-Platform Support
- ✅ Android - Full support with scoped storage
- ✅ iOS - Full support with app sandboxing
...
- ✅ Dark mode support
- ✅ Responsive design
- ✅ Comprehensive error messages (no sensitive data)
- ✅ Success confirmations with file preview
...
### Sign PDF with Digital Signature (NEW)
```dart
// Navigate to Sign PDF screen
Navigator.push(
  context,
  MaterialPageRoute(
    builder: (ctx) => const SignPdfScreenRefactored(),
  ),
);
